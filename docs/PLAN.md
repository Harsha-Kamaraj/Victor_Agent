...
| `SetActive` focuses a window | Windows' foreground lock downgrades it to flashing a taskbar button and reports no error, so `focus_app` claimed success while the user's editor stayed in front | Verify the foreground actually changed. The verification is the fix; a workaround that is not verified is how this got here |
| A `finally` releases what the chord pressed | The key was pressed before it was recorded, so a failure in between left a real modifier down and untracked — machine-wide on Windows | Record intent before acting, and release unconditionally as macOS does |
| `victor click` is a manual driver, so the user is the gate | UIA's Invoke on a file *opens* it, and the classifier read `setup.exe` as ordinary navigation — so the CLI would have run an installer with no prompt and no journal entry | Route the CLI through the same interceptor and journal as the agent, and classify labels that name executable files |
